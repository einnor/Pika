...
$(document).ready(function() {
	...
	loc = loc.split('/').splice(-1); 
	loc = loc.toString().split('.')[0].toString(); 
	$('.sidemenu').load('_side_menu.html',function(){
		$('.fa-bars').show();
		$('.fa-bars').click(function(){ $('.nav-side-menu').toggleClass('sidebarToggled'); });
		loadactivemenu( loc );
	});
});
...
function loadactivemenu(classname){
	...
	var datafind= $('.sidemenu').find('.'+classname).parents('ul').prop('id');
